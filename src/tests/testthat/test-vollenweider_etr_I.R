test_that("test-vollenweider_etr_I generate regression 20240925.csv", {
  ...
  if (os_name == "Linux") {
    ...
    expect_equal(model_result[["sdiff"]], 57.444503, tolerance=1e-4)
    expect_equal(model_result[["pmax"]], 165.46706, tolerance=1e-2)
    #expect_equal(model_result[["a"]], 0.001864811)
    #expect_equal(model_result[["alpha"]], 0.00001772)
    #expect_equal(model_result[["n"]], 42.4595519)
    expect_equal(model_result[["ik"]], 536.24841, tolerance=1e-2)
    expect_equal(model_result[["popt"]], 153.100940, tolerance=1e0)
    expect_equal(model_result[["iik"]], 496.172089, tolerance=1e-1)
    expect_equal(model_result[["pmax_popt_and_ik_iik_ratio"]], 1.0807710, tolerance=1e-5)
  } else {
...
test_that("test-vollenweider_etr_I generate regression modified 20240925.csv", {
  ...
  if (os_name == "Linux") {
    ...
    expect_equal(model_result[["sdiff"]], 57.444503, tolerance=1e-4)
    expect_equal(model_result[["a"]], 165.46706, tolerance=1e-2)
    #expect_equal(model_result[["b"]], 0.001864811)
    #expect_equal(model_result[["c"]], 0.00001772)
    #expect_equal(model_result[["d"]], 42.4595519)
    expect_equal(model_result[["alpha"]], 0.30856419, tolerance=1e-6)
    expect_equal(model_result[["beta"]], NA_real_)
    expect_equal(model_result[["etrmax_with_photoinhibition"]], 153.100940, tolerance=1e0)
    expect_equal(model_result[["etrmax_without_photoinhibition"]], 165.46706, tolerance=1e-2)
    expect_equal(model_result[["ik_with_photoinhibition"]], 496.172089, tolerance=1e-1)
    expect_equal(model_result[["ik_without_photoinhibition"]], 536.248419, tolerance=1e-2)
    expect_equal(model_result[["im_with_photoinhibition"]], 1420.0)
    expect_equal(model_result[["w"]], NA_real_)
    expect_equal(model_result[["ib"]], NA_real_)
    expect_equal(model_result[["etrmax_with_without_ratio"]], 1.0807710, tolerance=1e-5)
  } else {
    skip(paste("Unsupported operating system:", os_name))
  }
})
