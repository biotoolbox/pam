test_that("test-vollenweider_etr_II generate regression 20240925.csv", {
  ...
  if (os_name == "Linux") {
    ...
    expect_equal(model_result[["sdiff"]], 2.477197461, tolerance=1e-2)
    expect_equal(model_result[["pmax"]], 53.4558073, tolerance=1e-2)
    #expect_equal(model_result[["a"]], 0.002845961)
    #expect_equal(model_result[["alpha"]], -0.000378034)
    expect_equal(model_result[["n"]], 2.195311080, tolerance=1e-2)
    expect_equal(model_result[["ik"]], 351.808177, tolerance=1e-1)
    expect_equal(model_result[["popt"]], 44.1662558, tolerance=1e-2)
    expect_equal(model_result[["iik"]], 290.670944, tolerance=1e-3)
    expect_equal(model_result[["pmax_popt_and_ik_iik_ratio"]], 1.21033142, tolerance=1e-4)
  } else {
...
test_that("test-vollenweider_etr_II generate regression modified 20240925.csv", {
  ...
  if (os_name == "Linux") {
    ...
    expect_equal(model_result[["sdiff"]], 2.477197461, tolerance=1e-2)
    expect_equal(model_result[["a"]], 53.4558073, tolerance=1e-2)
    #expect_equal(model_result[["b"]], 0.002845961)
    #expect_equal(model_result[["c"]], -0.000378034)
    expect_equal(model_result[["d"]], 2.195311080, tolerance=1e-2)
    expect_equal(model_result[["alpha"]], 0.15194589, tolerance=1e-5)
    expect_equal(model_result[["beta"]], NA_real_)
    expect_equal(model_result[["etrmax_with_photoinhibition"]], 44.1662558, tolerance=1e-2)
    expect_equal(model_result[["etrmax_without_photoinhibition"]], 53.4558073, tolerance=1e-2)
    expect_equal(model_result[["ik_with_photoinhibition"]], 290.670944, tolerance=1e-3)
    expect_equal(model_result[["ik_without_photoinhibition"]], 351.808177, tolerance=1e-1)
    expect_equal(model_result[["im_with_photoinhibition"]], 756.0)
    expect_equal(model_result[["w"]], NA_real_)
    expect_equal(model_result[["ib"]], NA_real_)
    expect_equal(model_result[["etrmax_with_without_ratio"]], 1.21033142, tolerance=1e-4)
  } else {
    skip(paste("Unsupported operating system:", os_name))
  }
})
